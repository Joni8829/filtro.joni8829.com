    ...
    fn: (d, w, h) => {
      ...
      for (let i = 0; i < out.length; i += 4) {
        const shadow = Math.max(
          0,
          (120 - (out[i] + out[i + 1] + out[i + 2]) / 3) / 120,
        );
        out[i] += shadow * 28;
        out[i + 1] += shadow * 10;
        out[i + 2] -= shadow * 30;
  ...
  // ─── CINEMATIC ───────────────────────────────────────
  {
    ...
    fn: (d, w, h) => {
      ...
      for (let i = 0; i < out.length; i += 4) {
        ...
        if (lum < 120) {
          // Deep Teal Shadows
          out[i] -= ((120 - lum) / 120) * 20;
          out[i + 2] += ((120 - lum) / 120) * 25;
        } else {
          // Golden Highlights
          out[i] += ((lum - 120) / 135) * 30;
          out[i + 2] -= ((lum - 120) / 135) * 15;
        }
      }
      return out;
    },
...
function render() {
  ...
  // Grain overlay
  const totalGrain =
    (manualAdj.grain || 0) + getPresetParam(activeFilterId, "grain");
  if (totalGrain > 0) {
    ...
    for (let i = 0; i < gd.data.length; i += 4) {
      const n = (Math.random() - 0.5) * 255 * amount * 0.9;
  ...
  // Vignette
  const totalVig =
    (manualAdj.vignette || 0) + getPresetParam(activeFilterId, "vignette");
  if (totalVig > 0) {
...
function renderThumbnail(filter, thumbCanvas) {
  ...
  const vigAmount =
    filter.params && filter.params.vignette
      ? (filter.params.vignette / 100) * 0.7
      : 0.28;
...
function buildAdjPanel() {
  ...
  ADJ_DEFS.forEach(({ key, label, min, max }) => {
    ...
    row.querySelector("input").addEventListener("input", (e) => {
      ...
      document.getElementById(`adjval-${key}`).textContent =
        v > 0 ? "+" + v : v;
      render();
    });
  });
...
document.getElementById("dropZone").addEventListener("click", () => {
  if (!originalImage) triggerUpload();
});
